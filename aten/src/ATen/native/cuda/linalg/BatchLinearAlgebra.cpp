...
#else // USE_CUSOLVER|USE_HIPSOLVER && !AT_MAGMA_ENABLED
    lu_factor_cusolver(input, pivots, infos, compute_pivots);
#endif
#else // !USE_CUSOLVER && !USE_HIPSOLVER
    lu_factor_magma(input, pivots, infos, compute_pivots);
#endif
  }
...
void geqrf_kernel(const Tensor& input, const Tensor& tau) {
  std::cout << "GEQRF_KERNEL IS HIT!!!!!!!" << std::endl;
#if defined(CUDART_VERSION) || defined(USE_ROCM)
  std::cout << "MADE IT INTO ifdef!" << std::endl;
  auto geqrf_cusolver_backend = [](const Tensor& input, const Tensor& tau) {
#if defined(USE_CUSOLVER) || defined(USE_HIPSOLVER)
	std::cout << "MADE IT INTO LAMBDA FUNCTION" << std::endl;
      ...
      if (input.size(-2) <= 256 && batchCount(input) >= std::max<int64_t>(2, input.size(-2) / 16)) {
	    std::cout << "CALLING GEQRF_BATCHED_CUBLAS" << std::endl;
        return geqrf_batched_cublas(input, tau);
      } else {
	    std::cout << "CALLING GEQRF_CUSOLVER" << std::endl;
  ...
  // http://icl.cs.utk.edu/magma/forum/viewtopic.php?f=2&t=1015&p=2800&hilit=geqrf_gpu#p2800
    case at::LinalgBackend::Magma:
	  std::cout << "LINALG BACKEND IS MAGMA" << std::endl;
      return geqrf_magma(input, tau);
    case at::LinalgBackend::Cusolver:
    default:
	  std::cout << "BACKEND VERSION CALLED" << std::endl;
      return geqrf_cusolver_backend(input, tau);
  }
#else
  std::cout << "CALLING MAGMA VERSION" << std::endl;
  return geqrf_magma(input, tau);
#endif
}
